...
class CalendarView @JvmOverloads constructor(
    ...
    private fun initCalendarProperties(typedArray: TypedArray) = with(calendarProperties) {
        ...
        selectedCircleColor = typedArray.getColor(R.styleable.CalendarView_selectedCircleColor, 0)
        rangeColor = typedArray.getColor(R.styleable.CalendarView_rangeColor, 0)
        ...
        typeOfCalendar = typedArray.getInt(R.styleable.CalendarView_typeOfCalendar, FIXED_SET)
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            typeface = typedArray.getFont(R.styleable.CalendarView_typefacecalander)
    ...
    private fun initCalendar() {
        calendarPageAdapter =
            CalendarPageAdapter(
                ...
                callBack = {
                    calendarViewPager.setCurrentPosition(it)
                },
            )

        calendarViewPager.adapter = calendarPageAdapter
        calendarViewPager.layoutManager =
            LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        setUpCalendarPosition(Calendar.getInstance())
        calendarViewPager.setCalendarProperties(calendarProperties)
    }
//
//    /**
...
//    }
    ...
    private fun setUpCalendarPosition(calendar: Calendar) {
        ...
        calendarViewPager.setCurrentPosition(CalendarProperties.FIRST_VISIBLE_PAGE)
    }
    ...
    val currentPageDate: Calendar
        get() = (calendarProperties.firstPageCalendarDate.clone() as Calendar).apply {
            ...
            add(Calendar.MONTH, calendarViewPager.getCurrentPosition())
        }
    ...
    fun showCurrentMonthPage() {
        val page = calendarViewPager.getCurrentPosition() - midnightCalendar.getMonthsToDate(
            currentPageDate
        )
        calendarViewPager.scrollToPosition(page)
    }
    ...
    fun setDisabledWeekDays(disabledWeekDays: List<Int>) {
        calendarProperties.disabledWeekDays = disabledWeekDays
        calendarPageAdapter.notifyDataSetChanged()
    }

    fun getDisabledDays(): List<Calendar> {
        return calendarProperties.disabledDays
    }
    ...
    companion object {
        ...
        const val FIXED_SET = CLASSIC
        const val FLEXIBLE_SET = ONE_DAY_PICKER
        const val FIRST_DATE = 0
        const val LAST_DATE = 1
        const val VERY_FIRST_DATE = 2
    }
